...
Sketchpad::Sketchpad(const vec2& top_left_corner, size_t num_pixels_per_side,
    ...
    for(size_t i = 0; i < num_pixels_per_side ; i++) {
        for(size_t j = 0; j < num_pixels_per_side ; j++) {
            row.push_back(Pixel(kBlank));
        }
        
        current_image.AddRow(row);
    }
}
...
void Sketchpad::Draw() const {
  for (size_t row = 0; row < num_pixels_per_side_; ++row) {
    for (size_t col = 0; col < num_pixels_per_side_; ++col) {
      ...
      if (current_image.get_image_pixels()[row][col].IsShaded()) {
  ...
  for (size_t row = 0; row < num_pixels_per_side_; ++row) {
    for (size_t col = 0; col < num_pixels_per_side_; ++col) {
      ...
      if (glm::distance(brush_sketchpad_coords, pixel_center) <=
          brush_radius_) {
          current_image.SetPixel(Pixel(kBlack), row, col);
      }
    }
  }
}
...
void Sketchpad::Clear() {
  current_image.ClearImage();
    ...
    const Image &Sketchpad::getCurrentImage() const {
